...
async function checkCards(page: Page, section: "top" | "doctor", count: number) {
  ...
  expect(groupBox.y - (portrait.y + portrait.height)).toBeGreaterThanOrEqual(10);
  const boxes: Box[] = [];
  ...
  expect(boxes[1].x - (boxes[0].x + boxes[0].width)).toBeGreaterThanOrEqual(9);
}
...
for (const width of [320, 375, 390, 430, 640, 768, 820, 1024, 1280, 1440, 1920]) {
  test(`matching buttons and non-overlapping readable cards at ${width}px`, async ({ page }) => {
    ...
    expect(second.height).toBe(56);
    expect(Math.abs(first.width - second.width)).toBeLessThan(1);
    expect(overlaps(first, second)).toBe(false);
    for (const button of await buttons.all()) {
      await expect(button.locator(".hero-action__icon")).toHaveCSS("width", "32px");
      await expect(button.locator(".hero-action__icon")).toHaveCSS("height", "32px");
