...
import { CONCERNS, PLAN_OPTIONS as PLANS, SITE } from "@/content/site";
...
export function BookingSection() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        ...
        else setErrors({ form: data.error || "Please try again." });
        setStatus("error");
      ...
      setErrors({ form: "Network issue. Please try again or WhatsApp us directly." });
      setStatus("error");
      setTimeout(() => setStatus("idle"), 2500);
    }
  }
  ...
  return (
    <section id="book" className="relative py-20 sm:py-32 scroll-mt-20 overflow-hidden">
          ...
          <div className="lg:sticky lg:top-28">
            <Reveal>
              <p className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-saffron-300 to-saffron-500 text-forest-950 text-[10px] font-semibold tracking-[0.26em] uppercase px-5 py-2.5 shadow-lg">
                <CalendarCheck className="w-4 h-4" /> Book in 60 seconds
              </p>
            </Reveal>
            <Reveal delay={80}>
              <h2 className="mt-6 font-display font-light text-[clamp(2.2rem,4.8vw,3.5rem)] leading-[1.02] text-white">
                Your child&apos;s healing starts with <span className="italic text-saffron-300">one small form.</span>
              </h2>
            </Reveal>
            <Reveal delay={140}>
              <p className="mt-5 text-[16px] sm:text-[17.5px] leading-[1.75] text-white/65">
                Fill this patient enquiry form — it reaches <strong className="text-white">my personal email instantly</strong>,
                and my clinic calls you back within 24 hours with slots. No spam, no sharing, ever.
              </p>
            </Reveal>
            ...
            <Reveal delay={200}>
              <div className="mt-7 grid sm:grid-cols-2 gap-3">
                ...
                >
                  ...
                  <span>
                    <span className="block text-white font-bold text-sm">Online Consultation</span>
                    <span className="block text-white/55 text-xs mt-0.5">Video • India + Worldwide</span>
                  </span>
                </button>
                <button
                ...
                >
                  ...
                  <span>
                    <span className="block text-white font-bold text-sm">In-Clinic Visit</span>
                    <span className="block text-white/55 text-xs mt-0.5">Meet Dr. Priyanka in person</span>
                  </span>
                </button>
              </div>
            </Reveal>
            ...
            <Reveal delay={260}>
              <ul className="mt-7 space-y-3.5 rounded-[24px] bg-white/[0.05] backdrop-blur gold-hairline p-6">
                {[
                  { icon: Clock, t: SITE.hours, s: SITE.hoursNote },
                  { icon: Phone, t: SITE.phoneDisplay, s: "Call / WhatsApp • replies same day" },
                  { icon: Mail, t: SITE.email, s: "Form alerts land here instantly" },
                  { icon: MapPin, t: SITE.clinicName, s: SITE.addressFull },
                ].map((c) => (
                  <li key={c.t} className="flex items-center gap-3.5">
                    <span className="grid place-items-center w-10 h-10 rounded-xl bg-white/10 text-saffron-300 shrink-0"><c.icon className="w-4.5 h-4.5" /></span>
                    <span>
            ...
            <Reveal delay={300}>
              <div className="mt-4 flex items-center gap-2.5 text-xs text-white/55 px-1">
                ...
                Your details are encrypted & stored securely. Only the clinic team can access them.
              </div>
            </Reveal>
          </div>
          ...
          <Reveal variant="scale" delay={150}>
            <div className="relative rounded-[32px] bg-cream-50 gold-hairline shadow-luxe-lg overflow-hidden">
              <div className="bg-gradient-to-r from-saffron-300 via-saffron-400 to-saffron-500 px-6 sm:px-8 py-6 flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-display font-normal text-[22px] sm:text-[25px] text-forest-950 leading-tight">Patient Enquiry Form</h3>
                  <p className="text-[13px] font-medium text-forest-900/70">Takes ~60 seconds • Notifies doctor by email instantly ✉️</p>
                </div>
              </div>
              ...
              {status === "success" ? (
                <div className="px-6 sm:px-8 py-10 text-center" role="status" aria-live="polite">
                  ...
                  <h3 className="font-display font-light text-[28px] sm:text-[34px] text-forest-950">Thank you! We received it. 🌿</h3>
                  <p className="mt-3 text-[15px] text-forest-700 leading-relaxed max-w-md mx-auto">
                    Your enquiry <strong>#{refId}</strong> has been emailed to {SITE.doctorName}. Expect a personal
                    call/WhatsApp within <strong>24 hours</strong> (usually same day, Mon–Sat).
                  </p>
                  <div className="mt-6 rounded-2xl bg-emerald-50 border border-emerald-200 p-4 text-left max-w-md mx-auto space-y-2">
                    <p className="flex items-start gap-2 text-[13.5px] text-emerald-900"><CheckCircle2 className="w-4.5 h-4.5 shrink-0 mt-0.5" /> Keep your child&apos;s recent reports & growth records ready</p>
                    <p className="flex items-start gap-2 text-[13.5px] text-emerald-900"><CheckCircle2 className="w-4.5 h-4.5 shrink-0 mt-0.5" /> Note 2–3 photos if skin/allergy concern (helps a lot)</p>
                    <p className="flex items-start gap-2 text-[13.5px] text-emerald-900"><CheckCircle2 className="w-4.5 h-4.5 shrink-0 mt-0.5" /> Urgent? WhatsApp us directly — mention Ref #{refId}</p>
                  </div>
                  <button
                ...
                <form onSubmit={onSubmit} className="px-6 sm:px-8 py-7 space-y-5" noValidate aria-label="Patient enquiry form">
                  {/* Honeypot */}
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="parentName" className={labelCls}>Parent&apos;s name *</label>
                      <input id="parentName" value={form.parentName} onChange={(e) => set("parentName", e.target.value)} placeholder="e.g. Sneha Kulkarni" className={inputCls(errors.parentName)} autoComplete="name" required />
                    ...
                    <div>
                      ...
                      <input id="phone" value={form.phone} onChange={(e) => set("phone", e.target.value)} placeholder="+91 98765 43210" inputMode="tel" className={inputCls(errors.phone)} autoComplete="tel" required />
                      {errors.phone && <p className="mt-1 text-xs text-red-600 font-medium">{errors.phone}</p>}
                    </div>
                  </div>

                  <div>
                    <label htmlFor="email" className={labelCls}>Email <span className="font-normal text-forest-500">(for reports & summary)</span></label>
                    <input id="email" type="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="you@example.com" className={inputCls(errors.email)} autoComplete="email" />
                  ...
                  <div className="rounded-[22px] bg-gradient-to-b from-cream-100 to-cream-50 border border-saffron-500/15 p-5 space-y-4">
                    <p className="flex items-center gap-2 text-[13px] font-bold uppercase tracking-wider text-forest-700">
                      <Sparkles className="w-4 h-4 text-saffron-600" /> About your little one
                    </p>
                    <div className="grid sm:grid-cols-3 gap-4">
                      <div className="sm:col-span-1">
                        <label htmlFor="childName" className={labelCls}>Child&apos;s name *</label>
                        <input id="childName" value={form.childName} onChange={(e) => set("childName", e.target.value)} placeholder="e.g. Vihaan" className={inputCls(errors.childName)} required />
                        {errors.childName && <p className="mt-1 text-xs text-red-600 font-medium">{errors.childName}</p>}
                      </div>
                      <div>
                        <label htmlFor="childAge" className={labelCls}>Age *</label>
                        <input id="childAge" value={form.childAge} onChange={(e) => set("childAge", e.target.value)} placeholder="e.g. 4 yrs" className={inputCls(errors.childAge)} required />
                        {errors.childAge && <p className="mt-1 text-xs text-red-600 font-medium">{errors.childAge}</p>}
                      </div>
                      <div>
                        <label htmlFor="childGender" className={labelCls}>Gender</label>
                    ...
                    <div>
                      <label htmlFor="concern" className={labelCls}>Main concern *</label>
                    ...
                    <div>
                      <label htmlFor="symptoms" className={labelCls}>Symptoms in brief <span className="font-normal text-forest-500">(since when, what tried)</span></label>
                      <textarea id="symptoms" value={form.symptoms} onChange={(e) => set("symptoms", e.target.value)} rows={3} placeholder="e.g. Eczema patches behind knees since 1 yr, itching worse at night, tried 2 steroid creams…" className={cn(inputCls(), "resize-none")} />
                    </div>
                  </div>
                  ...
                  <div className="grid sm:grid-cols-3 gap-4">
                    <div>
                      ...
                      <div className="flex rounded-xl border-2 border-forest-100 overflow-hidden">
                        {([["online", "Online"], ["in_clinic", "Clinic"]] as const).map(([v, l]) => (
                          <button
                            type="button"
                            key={v}
                    ...
                    <div>
                      <label htmlFor="pdate" className={labelCls}>Preferred date</label>
                    ...
                    <div>
                      <label htmlFor="ptime" className={labelCls}>Preferred time</label>
                      <select id="ptime" value={form.preferredTime} onChange={(e) => set("preferredTime", e.target.value)} className={inputCls()}>
                        ...
                        <option>Afternoon (12–2)</option>
                        <option>Evening (5–7)</option>
                        <option>Night (7–8)</option>
                      </select>
                    </div>
                  </div>

                  <div>
                    <label htmlFor="plan" className={labelCls}>Interested plan</label>
                  ...
                  <div>
                    <label htmlFor="msg" className={labelCls}>Anything else? <span className="font-normal text-forest-500">(optional)</span></label>
                    <textarea id="msg" value={form.message} onChange={(e) => set("message", e.target.value)} rows={2} placeholder="Questions, urgency, reports available…" className={cn(inputCls(), "resize-none")} />
                  </div>
                  ...
                  <button
                    ...
                    className="btn-shine w-full inline-flex items-center justify-center gap-2.5 rounded-full bg-forest-950 hover:bg-forest-900 disabled:opacity-70 text-cream-50 font-medium text-[15.5px] tracking-wide px-6 py-4.5 shadow-luxe hover:shadow-luxe-lg transition-all duration-700"
                  >
                    {status === "sending" ? (
                      <>
                        <Loader2 className="w-5 h-5 animate-spin" /> Sending to doctor…
                      </>
                    ) : (
                      <>
                        <Send className="w-5 h-5 text-saffron-400" /> Send Enquiry to {SITE.doctorName.split(" ").slice(0, 2).join(" ")}
                      </>
                    )}
                  </button>
                  <p className="text-center text-xs text-forest-600 leading-relaxed">
                    By submitting, you agree to be contacted by the clinic. 🔒 Secure • No spam • Doctor notified by email instantly.
                  </p>
                </form>
              )}
            </div>
