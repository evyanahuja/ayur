...
import { METHOD_STEPS as STEPS, THERAPIES } from "@/content/site";



export function MethodShowcase() {
  const [tab, setTab] = useState(0);
  const t = THERAPIES[tab];

  return (
    <section id="method" className="relative py-20 sm:py-32 scroll-mt-24 overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <Reveal className="text-center max-w-3xl mx-auto">
          <p className="inline-flex items-center gap-2 rounded-full glass-dark text-saffron-300 text-[10px] font-semibold tracking-[0.28em] uppercase px-5 py-2.5">
            <Leaf className="w-3.5 h-3.5" /> The BalChikitsa Method™
          </p>
          <h2 className="mt-6 font-display font-light text-[clamp(2.1rem,4.6vw,3.4rem)] leading-[1.02] text-white">
            Not a 5-minute prescription. <span className="italic text-saffron-300">A complete healing journey.</span>
          </h2>
          <p className="mt-5 text-[16px] sm:text-[17.5px] text-white/60 leading-[1.75]">
            Transparent, trackable, and gentle — here&apos;s exactly what happens after you fill the form below.
          </p>
        </Reveal>

        {/* Steps */}
        <ol className="mt-12 grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
          {STEPS.map((s, i) => (
        ...
        {/* Therapy showcase */}
        <div className="mt-12 grid lg:grid-cols-[0.95fr_1.05fr] gap-6 items-stretch">
          <Reveal variant="left" className="rounded-[28px] overflow-hidden border border-white/10 bg-white/[0.05] backdrop-blur-xl">
            <div className="p-6 sm:p-8">
              <h3 className="font-display font-normal text-[25px] text-white">Signature therapies parents love</h3>
              <p className="mt-2 text-sm text-white/55">Tap to explore — all administered with child-safe dosing & guidance.</p>
              <div className="mt-6 grid gap-3" role="tablist" aria-label="Therapies">
                {THERAPIES.map((th, i) => (
                  <button
                    key={th.name}
                    role="tab"
          ...
          <Reveal variant="right" delay={120} className="relative rounded-[28px] overflow-hidden border border-white/10 min-h-[420px] group">
            ...
            <div className="absolute top-5 left-5 right-5 flex items-center justify-between">
              <span className="inline-flex items-center gap-2 rounded-full glass-dark text-white text-xs font-semibold px-4 py-2">
                <t.icon className="w-4 h-4 text-saffron-300" /> {t.tag}
              </span>
              <span className="rounded-full glass-dark text-saffron-300 text-xs font-bold px-3 py-2">
                {String(tab + 1).padStart(2, "0")} / 04
              </span>
            </div>
            <div className="absolute bottom-0 inset-x-0 p-6 sm:p-8" key={t.name}>
              <h4 className="font-display font-light text-[26px] sm:text-[32px] text-white leading-tight">{t.name}</h4>
              <p className="mt-2.5 text-[14px] sm:text-[15px] leading-relaxed text-white/75 max-w-xl">{t.desc}</p>
              <a href="#book" className="mt-6 btn-shine inline-flex items-center gap-2.5 rounded-full bg-white text-forest-950 font-medium tracking-wide text-[14px] px-6 py-3.5 hover:bg-saffron-300 transition-colors duration-500">
                Ask if this suits my child <ArrowRight className="w-4 h-4" />
              </a>
            </div>
          </Reveal>
        </div>
