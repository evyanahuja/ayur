...
/* ---------------------------------------------------------------------------
 * CredentialGrid — one normal-flow layout at every screen size.
 * Grid tracks give every card equal dimensions; long text wraps without ellipses.
 * No absolute offsets or independent transforms can collide with the portrait.
 * ------------------------------------------------------------------------ */
export function CredentialGrid({
  ...
  return (
    <div className="credential-group">
      <ul className="credential-grid" aria-label={label} role="list">
        {items.map((item) => (
          <li key={item.id} className="credential-card gold-hairline" data-credential={item.id}>
            <span
              aria-hidden="true"
              className={cn(
                "credential-card__icon",
                item.tone === "gold" && "credential-card__icon--gold",
                item.tone === "sage" && "credential-card__icon--sage"
              )}
            >
              {item.icon}
            </span>
            <span className="credential-card__copy">
              <span className="credential-card__title">{item.title}</span>
              <span className="credential-card__subtitle">{item.subtitle}</span>
            </span>
          </li>
        ))}
      </ul>
