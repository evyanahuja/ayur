...
import { useRegionCurrency } from "@/lib/useRegionCurrency";

/** Swap the price suffix of a translated label with the live price from site.ts PLANS. */
function withLivePrice(label: string, price: string): string {
  if (/—\s*[₹$][\d,]+/.test(label)) return label.replace(/—\s*[₹$][\d,]+/, `— ${price}`);
  return `${label} — ${price}`;
}
...
export function BookingSection() {
  ...
  // Single source of truth: prices always come from PLANS in site.ts.
  // Translated labels keep their wording; only the price suffix is swapped.
  const livePrice = (id: string): string => {
    const plan = PLANS.find((p) => p.id === id);
    if (!plan) return "";
    return currency === "USD" ? plan.usdPrice : plan.price;
  };
  const planOptions = [
    withLivePrice(t.booking.planOptions[0], livePrice("first")),
    withLivePrice(t.booking.planOptions[1], livePrice("wellness")),
    withLivePrice(t.booking.planOptions[2], livePrice("followup")),
    t.booking.planOptions[3],
    t.booking.planOptions[4],
  ];
                ...
                <form onSubmit={onSubmit} className="px-6 sm:px-8 py-7 space-y-5" noValidate aria-label={t.booking.formTitle}>
                  ...
                  <div>
                    ...
                    <select id="plan" value={form.planInterest} onChange={(e) => set("planInterest", e.target.value)} className={inputCls()}>
                      {PLAN_KEYS.map((k, i) => (
                        <option key={k} value={k}>{planOptions[i]}</option>
                      ))}
                    </select>
                  </div>
