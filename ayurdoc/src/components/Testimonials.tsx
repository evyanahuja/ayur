...
export function Testimonials() {
  ...
  const items = TESTIMONIALS.map((base, i) => ({
    ...base,
    ...t.testimonials.items[i],
    img: base.img,
    stars: base.stars,
  }));
  const wins = t.testimonials.miniWins;
  ...
  return (
    <section id="stories" className="mandala-section relative py-12 sm:py-16 scroll-mt-24 overflow-hidden">
      <SectionMandalas id="stories" palette="lotus" side="left" />
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <Reveal className="text-center max-w-3xl mx-auto">
          <p className="inline-flex items-center gap-2 rounded-full bg-rose-500/10 border border-rose-300/40 text-rose-700 text-[11px] font-semibold tracking-[0.24em] uppercase px-4 py-2">
            <Heart className="w-3.5 h-3.5" fill="currentColor" /> {STATS.patientsTreated.toLocaleString("en-IN")}+ {t.testimonials.eyebrow}
          </p>
          <h2 className="mt-6 font-display font-light text-[clamp(2.1rem,4.6vw,3.4rem)] leading-[1.02] text-forest-950">
            {t.testimonials.headingPre} <span className="italic text-gradient-gold">{t.testimonials.headingHighlight}</span>
          </h2>
        ...
        <Reveal delay={120} className="mt-9">
          ...
          >
              ...
              >
                {items.map((item) => (
                  <article
                    ...
                    aria-label={`${t.testimonials.from} ${item.name}`}
                  >
                      ...
                      <div className="text-center sm:text-left">
                        ...
                        <div className="mt-4 flex sm:flex-col items-center sm:items-start gap-4">
                          ...
                          <div>
                            ...
                            <div className="mt-2 flex items-center gap-0.5 justify-center sm:justify-start" aria-label={`${item.stars} stars`}>
            ...
            <div className="mt-6 flex items-center justify-center gap-4">
              ...
              <div className="flex items-center gap-2" role="tablist" aria-label={t.testimonials.chooseStory}>
                {items.map((_, i) => (
                  <button
                    key={i}
                    role="tab"
